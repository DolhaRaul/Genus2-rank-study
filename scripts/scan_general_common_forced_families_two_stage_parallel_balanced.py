...
# Number of rs-tuples to generate and sort for each m. Use None to test all combinations.
# With MAX_Q_PER_RS below, a large value is not harmful, but values above the number
# effectively reached by MAX_FORCED_FAMILIES will not be used.
...
def make_monic_poly_from_tail(tail_coeffs, deg):
    """
    deg=2, [B,C] -> x^2 + B*x + C
    deg=1, [D]   -> x + D
    deg=0, []    -> 1
...
def build_family_general(rs, q, L_tail_coeffs):
    """
    For fixed rs, q and monic Q/L of degree 6-len(rs), construct:
        H  = prod (x-r_i)
        Pq = q^2 mod H
        L  = monic polynomial of degree 6-m
        F  = Pq + H*L
    """
    ...
    degL = 6 - m
    L = make_monic_poly_from_tail(L_tail_coeffs, degL)
    F = R(Pq + H * L)
...
def scan_one_general_family(m, rs, q, q_params, q_coeffs,
    ...
    for L_tail in L_tail_grid_for_m(m, L_param_pool):
        data = build_family_general(rs=rs, q=q, L_tail_coeffs=L_tail)
